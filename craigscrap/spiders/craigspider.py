...
def make_url_list():
    places = ['sacramento', 'humboldt']
    new_urls = []
    for place_name in places:
        # splitted_url = url.split('/')
        # splitted_name = splitted_url[2].split('.')
        # place_name = splitted_name[0]
        new_url = 'http://webcache.googleusercontent.com/search?q=cache:' + place_name + '.craigslist.org/search/cto'
...
class CraigSpider(scrapy.Spider):
    ...
    # start_urls = ['http://webcache.googleusercontent.com/search?q=cache:geo.craigslist.org/iso/us/ca']
    start_urls = make_url_list()
